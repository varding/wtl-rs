...
//#ifndef _WIN32_WCE
	...
	pub fn GetIcon (&self)->HICON {
		self.inner.assert_window();
		self.inner.SendMessage( STM_GETICON, 0, 0) as HICON
	}

	pub fn SetIcon(&self,hIcon: HICON)->HICON {
		self.inner.assert_window();
		self.inner.SendMessage( STM_SETICON, hIcon as WPARAM, 0) as HICON
	}

	pub fn GetEnhMetaFile (&self)->HENHMETAFILE {
		self.inner.assert_window();
		self.inner.SendMessage( STM_GETIMAGE, IMAGE_ENHMETAFILE as WPARAM, 0) as HENHMETAFILE
	}

	pub fn SetEnhMetaFile(&self,hMetaFile: HENHMETAFILE)->HENHMETAFILE {
		self.inner.assert_window();
		self.inner.SendMessage( STM_SETIMAGE, IMAGE_ENHMETAFILE as WPARAM, hMetaFile as LPARAM) as HENHMETAFILE
	}
//#else // CE specific
	// pub fn GetIcon (&self)->HICON {
	// 	self.inner.assert_window();
	// 	self.inner.SendMessage( STM_GETIMAGE, IMAGE_ICON, 0) as HICON
	// }

	// pub fn SetIcon(&self,hIcon: HICON)->HICON {
	// 	self.inner.assert_window();
	// 	self.inner.SendMessage( STM_SETIMAGE, IMAGE_ICON, hIcon as LPARAM) as HICON
	// }
//#endif // _WIN32_WCE

	// pub fn GetBitmap ()->CBitmapHandle {
	// 	self.inner.assert_window();
	// 	return CBitmapHandle((HBITMAP)self.inner.SendMessage( STM_GETIMAGE, IMAGE_BITMAP, 0));
	// }

	// pub fn SetBitmap(&self,hBitmap: HBITMAP)->CBitmapHandle {
	// 	self.inner.assert_window();
	// 	return CBitmapHandle((HBITMAP)self.inner.SendMessage( STM_SETIMAGE, IMAGE_BITMAP, hBitmap as LPARAM));
	// }

	pub fn GetCursor (&self)->HCURSOR {
		self.inner.assert_window();
		self.inner.SendMessage( STM_GETIMAGE, IMAGE_CURSOR as WPARAM, 0) as HCURSOR
	}

	pub fn SetCursor(&self,hCursor: HCURSOR)->HCURSOR {
		self.inner.assert_window();
		self.inner.SendMessage( STM_SETIMAGE, IMAGE_CURSOR as WPARAM, hCursor as LPARAM) as HCURSOR
	}
}
