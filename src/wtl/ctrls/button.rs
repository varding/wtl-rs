...
// Attributes
	...
	pub fn GetState(&self) ->UINT {
		//self.inner.assert_window();
		//return (UINT)self.inner.SendMessage( BM_GETSTATE, 0, 0);
		self.inner.assert_window();
		self.inner.SendMessage(BM_GETSTATE, 0, 0) as UINT
	}

	pub fn SetState(&self,bHighlight: BOOL) {
		self.inner.assert_window();
		self.inner.SendMessage( BM_SETSTATE, bHighlight as WPARAM, 0);
	}

	pub fn GetCheck(&self)->c_int {
		self.inner.assert_window();
		self.inner.SendMessage( BM_GETCHECK, 0, 0) as c_int
	}

	pub fn SetCheck(&self,nCheck: c_int) {
		self.inner.assert_window();
		self.inner.SendMessage( BM_SETCHECK, nCheck as WPARAM, 0);
	}
	...
	pub fn SetButtonStyle(&self,nStyle:UINT, bRedraw:BOOL) {
		self.inner.assert_window();
		self.inner.SendMessage( BM_SETSTYLE, nStyle as WPARAM, bRedraw as LPARAM);
	}

//#ifndef _WIN32_WCE
	pub fn GetIcon(&self)->HICON {
		self.inner.assert_window();
		self.inner.SendMessage( BM_GETIMAGE, IMAGE_ICON as WPARAM, 0) as HICON
	}

	pub fn SetIcon(&self,hIcon: HICON)->HICON {
		self.inner.assert_window();
		self.inner.SendMessage( BM_SETIMAGE, IMAGE_ICON as WPARAM, hIcon as LPARAM) as HICON
	}

	// pub fn GetBitmap ()->CBitmapHandle 
	// {
	// 	self.inner.assert_window();
	// 	return CBitmapHandle((HBITMAP)self.inner.SendMessage( BM_GETIMAGE, IMAGE_BITMAP, 0));
	// }

	// pub fn SetBitmap(&self,hBitmap: HBITMAP)->CBitmapHandle
	// {
	// 	self.inner.assert_window();
	// 	return CBitmapHandle((HBITMAP)self.inner.SendMessage( BM_SETIMAGE, IMAGE_BITMAP, hBitmap as LPARAM));
	// }
//#endif // !_WIN32_WCE
...
//#if (WINVER >= 0x0600)
	pub fn SetDontClick(&self,bDontClick: BOOL) {
		self.inner.assert_window();
		self.inner.SendMessage( BM_SETDONTCLICK, bDontClick as WPARAM, 0);
	}
//#endif // (WINVER >= 0x0600)
...
// Operations
	pub fn Click (&self) {
		self.inner.assert_window();
		self.inner.SendMessage( BM_CLICK, 0, 0);
	}
}
