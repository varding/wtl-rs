...
// 	}
	...
	pub fn CanUndo(&self)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_CANUNDO, 0, 0) as BOOL
	}

	pub fn GetLineCount(&self)->c_int {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETLINECOUNT, 0, 0) as c_int
	}

	pub fn GetModify(&self)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETMODIFY, 0, 0) as BOOL
	}

	pub fn SetModify (&self,bModified: BOOL) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETMODIFY,  bModified as WPARAM, 0);
	}

	pub fn GetRect(&self,lpRect: LPRECT) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETRECT, 0, lpRect as LPARAM);
	}

	pub fn GetSel(&self)->DWORD {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETSEL, 0, 0) as DWORD
	}

	pub fn GetSel2 (&self,nStartChar: &mut c_int, nEndChar: &mut c_int) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETSEL, nStartChar as *mut _ as *mut c_void as WPARAM, nEndChar as *mut _ as *mut c_void as LPARAM);
	}

//#ifndef _WIN32_WCE
	pub fn GetHandle(&self)->HLOCAL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETHANDLE, 0, 0) as HLOCAL
	}

	pub fn SetHandle(&self,hBuffer: HLOCAL) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETHANDLE, hBuffer as WPARAM, 0);
	}
//#endif // !_WIN32_WCE

	pub fn GetMargins(&self)->DWORD {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETMARGINS, 0, 0) as DWORD
	}

	pub fn GetMargins2 (&self,nLeft: &mut UINT, nRight: &mut UINT) {
		self.inner.assert_window();
		let dwRet = self.inner.SendMessage( EM_GETMARGINS, 0, 0) as DWORD;
	...
	pub fn SetMargins (&self, nLeft: UINT, nRight: UINT, wFlags: WORD) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETMARGINS,  wFlags as WPARAM, MAKELONG(nLeft as WORD, nRight as WORD) as LPARAM);
	}

	pub fn GetLimitText(&self)->UINT {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETLIMITTEXT, 0, 0) as UINT
	}

	pub fn SetLimitText(&self,nMax: UINT) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETLIMITTEXT,  nMax as WPARAM, 0);
	}

	pub fn PosFromChar(&self,nChar: UINT)->POINT {
		self.inner.assert_window();
		let dwRet = self.inner.SendMessage( EM_POSFROMCHAR,  nChar as WPARAM, 0) as DWORD;
	...
	pub fn CharFromPos (&self, pt: POINT, pLine: Option<&mut c_int>)->c_int {
		self.inner.assert_window();
		let dwRet = self.inner.SendMessage( EM_CHARFROMPOS, 0, MAKELONG(pt.x as WORD, pt.y as WORD) as LPARAM) as DWORD;
		// if pLine != None{
...
//#ifndef _WIN32_WCE
	pub fn GetWordBreakProc(&self)->EDITWORDBREAKPROCW {
		self.inner.assert_window();
		unsafe{
			::std::mem::transmute(self.inner.SendMessage( EM_GETWORDBREAKPROC, 0, 0))
		}
	}

	pub fn SetWordBreakProc(&self,ewbprc: EDITWORDBREAKPROCW) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETWORDBREAKPROC, 0, ewbprc as LPARAM);
	}
//#endif // !_WIN32_WCE

	pub fn GetFirstVisibleLine(&self)->c_int {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETFIRSTVISIBLELINE, 0, 0) as c_int
	}
...
//#ifndef _WIN32_WCE
	pub fn GetThumb(&self)->c_int {
		...
		self.inner.SendMessage( EM_GETTHUMB, 0, 0) as c_int
	}
//#endif // !_WIN32_WCE

	pub fn SetReadOnly (&self, bReadOnly: BOOL)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETREADONLY,  bReadOnly as WPARAM, 0) as BOOL
	}

//#if (WINVER >= 0x0500) && !defined(_WIN32_WCE)
	pub fn GetImeStatus(&self,uStatus: UINT)->UINT {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETIMESTATUS,  uStatus as WPARAM, 0) as UINT
	}

	pub fn SetImeStatus(&self,uStatus: UINT, uData: UINT)->UINT {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETIMESTATUS,  uStatus as WPARAM,  uData as LPARAM) as UINT
	}
...
//#if (_WIN32_WINNT >= 0x0501)
	pub fn GetCueBannerText(&self,lpstrText: LPCWSTR, cchText: c_int)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETCUEBANNER, lpstrText as WPARAM,  cchText as LPARAM) as BOOL
	}
...
// Operations
	pub fn EmptyUndoBuffer(&self) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_EMPTYUNDOBUFFER, 0, 0);
	}

	pub fn FmtLines(&self,bAddEOL: BOOL)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_FMTLINES,  bAddEOL as WPARAM, 0) as BOOL
	}

	pub fn LimitText (&self,nChars: c_int) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_LIMITTEXT,  nChars as WPARAM, 0);
	}

	pub fn LineFromChar(&self,nIndex: c_int)->c_int {
		self.inner.assert_window();
		self.inner.SendMessage( EM_LINEFROMCHAR,  nIndex as WPARAM, 0) as c_int
	}

	pub fn LineIndex(&self,nLine: c_int)->c_int {
		self.inner.assert_window();
		self.inner.SendMessage( EM_LINEINDEX,  nLine as WPARAM, 0) as c_int
	}

	pub fn LineLength(&self,nLine: c_int)->c_int {
		self.inner.assert_window();
		self.inner.SendMessage( EM_LINELENGTH,  nLine as WPARAM, 0) as c_int
	}

	pub fn LineScroll(&self,nLines: c_int, nChars: c_int) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_LINESCROLL,  nChars as WPARAM,  nLines as LPARAM);
	}
	...
	pub fn SetRect(&self,lpRect: LPCRECT) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETRECT, 0, lpRect as LPARAM);
	}

	pub fn SetRectNP(&self,lpRect: LPCRECT) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETRECTNP, 0, lpRect as LPARAM);
	}

	pub fn SetSel(&self,dwSelection: DWORD, bNoScroll: BOOL) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETSEL, LOWORD(dwSelection) as WPARAM, HIWORD(dwSelection) as LPARAM);
		if bNoScroll == FALSE {
			self.inner.SendMessage( EM_SCROLLCARET, 0, 0);
		}
	}

	pub fn SetSel2(&self,nStartChar: c_int, nEndChar: c_int,bNoScroll: BOOL) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETSEL,  nStartChar as WPARAM,  nEndChar as LPARAM);
		if bNoScroll == FALSE{
			self.inner.SendMessage( EM_SCROLLCARET, 0, 0);
		}
	}
	...
	pub fn SetTabStops3(&self,nTabStops: c_int, rgTabStops: LPINT)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETTABSTOPS,  nTabStops as WPARAM, rgTabStops as LPARAM) as BOOL
	}

	pub fn SetTabStops(&self)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETTABSTOPS, 0, 0) as BOOL
	}

	pub fn SetTabStops2(&self,cxEachStop: *const c_int)->BOOL	{
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETTABSTOPS, 1,cxEachStop as LPARAM) as BOOL
	}
	...
	pub fn Scroll(&self,nScrollAction: c_int)->c_int {
		self.inner.assert_window();
		debug_assert!((self.inner.GetStyle() & ES_MULTILINE) != 0);
		let lRet = self.inner.SendMessage( EM_SCROLL,  nScrollAction as WPARAM, 0);
...
//#if (_WIN32_WINNT >= 0x0501)
	pub fn ShowBalloonTip(&self,pEditBaloonTip: PEDITBALLOONTIP)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SHOWBALLOONTIP, 0, pEditBaloonTip as LPARAM) as BOOL
	}

	pub fn HideBalloonTip(&self)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_HIDEBALLOONTIP, 0, 0) as BOOL
	}
//#endif // (_WIN32_WINNT >= 0x0501)
...
//#if (_WIN32_WINNT >= 0x0600)
	pub fn GetHilite(&self)->DWORD {
		self.inner.assert_window();
		self.inner.SendMessage( EM_GETHILITE, 0, 0) as DWORD
	}

	pub fn GetHilite2(&self, nStartChar: &mut c_int, nEndChar: &mut c_int) {
		self.inner.assert_window();
		let dwRet = self.inner.SendMessage( EM_GETHILITE, 0, 0) as DWORD;
	...
	pub fn SetHilite(&self,nStartChar: c_int, nEndChar: c_int) {
		self.inner.assert_window();
		self.inner.SendMessage( EM_SETHILITE,  nStartChar as WPARAM,  nEndChar as LPARAM);
	}
//#endif // (_WIN32_WINNT >= 0x0600)
	...
	pub fn Undo(&self)->BOOL {
		self.inner.assert_window();
		self.inner.SendMessage( EM_UNDO, 0, 0) as BOOL
	}
