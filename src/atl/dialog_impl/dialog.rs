            ...
            match uMsg {
                ...
                _ => {
                    if mut_self.state & WINSTATE_DESTROYED == 0 {
                        user32::SetWindowLongPtrW(mut_self.inner.GetHwnd(),DWLP_MSGRESULT as c_int,lRes);
                    }
                }
            }
...
impl<T> Dialog<T> {
    ...
    pub fn EndDialog(&self, nRetCode: c_int) -> BOOL {
        ...
        assert!(self.modal);
        unsafe {
            user32::EndDialog(self.inner.GetHwnd(), nRetCode as INT_PTR)
        }
    }
    ...
    //     // unsafe {
    //     //     if user32::DestroyWindow(self.inner.GetHwnd()) == FALSE {
    //     //         return FALSE;
    //     //     }
    //     // }
    //     // return TRUE;
