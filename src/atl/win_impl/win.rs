...
impl<T> CWindowImpl<T> {
    pub fn new()->CWindowImpl<T> {
    	CWindowImpl {
    		super_win_proc: Some(user32::DefWindowProcW),
...
impl<T> CWindowImpl<T> {
 //    fn DefWindowProc2(&self)->LRESULT	{
	...
	fn DefWindowProc(&self,uMsg: UINT,wParam: WPARAM,lParam: LPARAM)->LRESULT {
		unsafe {
			user32::CallWindowProcW(self.super_win_proc, self.inner.GetHwnd(), uMsg, wParam, lParam)
		}
...
impl<T> CWindowImpl<T> {
	...
	*/
    ...
    unsafe extern "system" fn WindowProc(hWnd: HWND,uMsg: UINT,wParam: WPARAM,lParam: LPARAM)->LRESULT {
		...
		if bRet == FALSE {
			if uMsg != WM_NCDESTROY {
				...
				// unsubclass, if needed
				// let pfnWndProc = user32::GetWindowLongPtrW(h, GWLP_WNDPROC);
				// lRes = Self::DefWindowProc(mut_self,uMsg, wParam, lParam);
				// if let Some(p) = mut_self.super_win_proc {
				// 	if p != user32::DefWindowProcW && user32::GetWindowLongPtrW(h, GWLP_WNDPROC) == pfnWndProc {
				// 		user32::SetWindowLongPtrW(h, GWLP_WNDPROC, mut_self.super_win_proc as LONG_PTR);
				// 	}
				// }
				// mark window as destryed
    ...
    pub fn SubclassWindow(&mut self,hWnd: HWND)->BOOL {
		...
		let pfnWndProc = unsafe{ user32::SetWindowLongPtrW(hWnd,
