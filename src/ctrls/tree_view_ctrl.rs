...
impl CTreeViewCtrl {
	...
	pub fn GetIndent(&self,)->UINT {
		self.cwin.SendMessage(TVM_GETINDENT, 0, 0) as UINT
	}

	pub fn SetIndent(&self,nIndent: UINT,) {
		self.cwin.SendMessage(TVM_SETINDENT, nIndent as WPARAM, 0);
	}

	// pub fn GetImageList(nImageListType: Option<c_int> /*= TVSIL_NORMAL*/,)->CImageList {
	// 	self.cwin.assert_window();
	// 	CImageList((HIMAGELIST)self.cwin.SendMessage(TVM_GETIMAGELIST, (WPARAM)nImageListType, 0))
	// }

	// pub fn SetImageList(&self,hImageList: HIMAGELIST, nImageListType: Option<c_int> /*= TVSIL_NORMAL*/,)->CImageList {
	// 	self.cwin.assert_window();
	// 	CImageList((HIMAGELIST)self.cwin.SendMessage(TVM_SETIMAGELIST, (WPARAM)nImageListType, (LPARAM)hImageList))
	// }

	pub fn GetItem(&self,pItem: LPTVITEMW,)->BOOL {
		self.cwin.SendMessage(TVM_GETITEMW, 0, pItem as LPARAM) as BOOL
	}

	pub fn SetItem2(&self,pItem: LPTVITEMW,)->BOOL {
		self.cwin.SendMessage(TVM_SETITEMW, 0, pItem as LPARAM) as BOOL
	}
	...
	pub fn SetItem(&self,hItem: HTREEITEM, nMask: UINT, lpszItem: Option<&str>, nImage: c_int, nSelectedImage: c_int, nState: UINT, nStateMask: UINT, lParam: LPARAM,)->BOOL {
		...
		item.lParam = lParam;
		self.cwin.SendMessage(TVM_SETITEMW, 0, &item as *const _ as LPARAM) as BOOL
	}
...
//#ifndef _ATL_NO_COM
	pub fn GetItemText(&self,hItem: HTREEITEM, bstrText: &BSTR,)->BOOL {
		...
		for(c_int nLen = 256; ; nLen *= 2)
		{
			...
			bRet = (BOOL)self.cwin.SendMessage(TVM_GETITEM, 0, (LPARAM)&item);
...
//#endif // defined(_WTL_USE_CSTRING) || defined(__ATLSTR_H__)
	...
	pub fn GetItemImage(&self,hItem: HTREEITEM, nImage: &mut c_int,nSelectedImage: &mut c_int,)->BOOL {
		...
		let bRes = self.cwin.SendMessage(TVM_GETITEMW, 0, &item as *const _ as LPARAM) as BOOL;
		if bRes > 0	{
			*nImage = item.iImage;
...
//#if (_WIN32_IE >= 0x0500) && !defined(_WIN32_WCE)
		(self.cwin.SendMessage(TVM_GETITEMSTATE, hItem as WPARAM, nStateMask as LPARAM) as UINT) & nStateMask
//#else // !((_WIN32_IE >= 0x0500) && !defined(_WIN32_WCE))
		...
		// item.state = 0;
		// item.stateMask = nStateMask;
		// self.cwin.SendMessage(TVM_GETITEM, 0, (LPARAM)&item);
...
//#endif // !((_WIN32_IE >= 0x0500) && !defined(_WIN32_WCE))
	...
	pub fn GetItemData(&self,hItem: HTREEITEM,)->DWORD_PTR {
		...
		let bRet = self.cwin.SendMessage(TVM_GETITEMW, 0, &item as *const _ as LPARAM) as BOOL;
	...
	// 	CEdit(self.cwin.SendMessage(TVM_GETEDITCONTROL, 0, 0) as HWND)
	// }

	pub fn GetVisibleCount(&self,)->UINT {
		self.cwin.SendMessage(TVM_GETVISIBLECOUNT, 0, 0) as UINT
	}
	...
	pub fn GetItemRect(&self,hItem: HTREEITEM, lpRect: LPRECT, bTextOnly: BOOL,)->BOOL {
		...
		self.cwin.SendMessage(TVM_GETITEMRECT, bTextOnly as WPARAM, lpRect as LPARAM) as BOOL
	}
	...
	pub fn ItemHasChildren(&self,hItem: HTREEITEM,)->BOOL {
		...
		self.cwin.SendMessage(TVM_GETITEMW, 0, &item as *const _ as LPARAM);
		item.cChildren
	}

//#ifndef _WIN32_WCE
	// pub fn GetToolTips(&self,)->CToolTipCtrl {
	// 	CToolTipCtrl((HWND)self.cwin.SendMessage(TVM_GETTOOLTIPS, 0, 0))
	// }

	// pub fn SetToolTips(&self,hWndTT: HWND,)->CToolTipCtrl {
	// 	CToolTipCtrl((HWND)self.cwin.SendMessage(TVM_SETTOOLTIPS, hWndTT as WPARAM, 0))
	// }
//#endif // !_WIN32_WCE

	// pub fn GetISearchString(&self,lpstr: LPTSTR,)->c_int {
	// 	self.cwin.SendMessage(TVM_GETISEARCHSTRING, 0, lpstr as LPARAM) as c_int
	// }

	// checkboxes only
	pub fn GetCheckState(&self,hItem: HTREEITEM,)->BOOL {
		debug_assert!((self.cwin.GetStyle() & TVS_CHECKBOXES) != 0);
...
//#if (_WIN32_IE >= 0x0400) && !defined(_WIN32_WCE)
	pub fn GetBkColor(&self,)->COLORREF {
		self.cwin.SendMessage(TVM_GETBKCOLOR, 0, 0) as COLORREF
	}

	pub fn SetBkColor(&self,clr: COLORREF,)->COLORREF {
		self.cwin.SendMessage(TVM_SETBKCOLOR, 0, clr as LPARAM) as COLORREF
	}

	pub fn GetInsertMarkColor(&self,)->COLORREF {
		self.cwin.SendMessage(TVM_GETINSERTMARKCOLOR, 0, 0) as COLORREF
	}

	pub fn SetInsertMarkColor(&self,clr: COLORREF,)->COLORREF {
		self.cwin.SendMessage(TVM_SETINSERTMARKCOLOR, 0, clr as LPARAM) as COLORREF
	}

	pub fn GetItemHeight(&self,)->c_int {
		self.cwin.SendMessage(TVM_GETITEMHEIGHT, 0, 0) as c_int
	}

	pub fn SetItemHeight(&self,cyHeight: c_int,)->c_int {
		self.cwin.SendMessage(TVM_SETITEMHEIGHT, cyHeight as WPARAM, 0) as c_int
	}

	pub fn GetScrollTime(&self,)->c_int {
		self.cwin.SendMessage(TVM_GETSCROLLTIME, 0, 0) as c_int
	}

	pub fn SetScrollTime(&self,nScrollTime: c_int,)->c_int {
		self.cwin.SendMessage(TVM_SETSCROLLTIME, nScrollTime as WPARAM, 0) as c_int
	}

	pub fn GetTextColor(&self,)->COLORREF {
		self.cwin.SendMessage(TVM_GETTEXTCOLOR, 0, 0) as COLORREF
	}

	pub fn SetTextColor(&self,clr: COLORREF,)->COLORREF {
		self.cwin.SendMessage(TVM_SETTEXTCOLOR, 0, clr as LPARAM) as COLORREF
	}

	pub fn GetUnicodeFormat(&self,)->BOOL {
		self.cwin.SendMessage(TVM_GETUNICODEFORMAT, 0, 0) as BOOL
	}

	pub fn SetUnicodeFormat(&self,bUnicode: Option<BOOL> /*= TRUE*/,)->BOOL {
		let b = if let Some(u) = bUnicode {u}else{TRUE};
		self.cwin.SendMessage(TVM_SETUNICODEFORMAT, b as WPARAM, 0) as BOOL
	}
//#endif // (_WIN32_IE >= 0x0400) && !defined(_WIN32_WCE)

//#if (_WIN32_IE >= 0x0500) && !defined(_WIN32_WCE)
	pub fn GetLineColor(&self,)->COLORREF {
		self.cwin.SendMessage(TVM_GETLINECOLOR, 0, 0) as COLORREF
	}

	pub fn SetLineColor(&self, clrNew: COLORREF /*= CLR_DEFAULT*/,)->COLORREF {
		self.cwin.SendMessage(TVM_SETLINECOLOR, 0, clrNew as LPARAM) as COLORREF
	}
//#endif // (_WIN32_IE >= 0x0500) && !defined(_WIN32_WCE)
...
//#endif
		self.cwin.SendMessage(TVM_GETEXTENDEDSTYLE, 0, 0) as DWORD
	}
...
//#endif
		self.cwin.SendMessage(TVM_SETEXTENDEDSTYLE, dwMask as WPARAM, dwStyle  as LPARAM) as DWORD
	}

//#if (_WIN32_WINNT >= 0x0600)
	pub fn SetAutoScrollInfo(&self,uPixPerSec: UINT, uUpdateTime: UINT,)->BOOL {
		self.cwin.SendMessage(TVM_SETAUTOSCROLLINFO, uPixPerSec as WPARAM, uUpdateTime as LPARAM) as BOOL
	}

	pub fn GetSelectedCount(&self,)->DWORD {
		self.cwin.SendMessage(TVM_GETSELECTEDCOUNT, 0, 0) as DWORD
	}
	...
	pub fn GetItemPartRect(&self,hItem: HTREEITEM, partID: TVITEMPART, lpRect: LPRECT,)->BOOL {
		...
		self.cwin.SendMessage(TVM_GETITEMPARTRECT, 0, &gipri as *const _ as LPARAM) as BOOL
	}
//#endif // (_WIN32_WINNT >= 0x0600)

// Operations
	pub fn InsertItem2(&self,lpInsertStruct: LPTVINSERTSTRUCTW,)->HTREEITEM {
		self.cwin.SendMessage(TVM_INSERTITEMW, 0, lpInsertStruct as LPARAM) as HTREEITEM
	}
	...
	pub fn InsertItem(&self,nMask: UINT, lpszItem: Option<&str>, nImage: c_int, nSelectedImage: c_int, nState: UINT, nStateMask: UINT, lParam: LPARAM, hParent: HTREEITEM, hInsertAfter: HTREEITEM,)->HTREEITEM {
		...
		self.cwin.SendMessage(TVM_INSERTITEMW, 0, &tvis as *const _ as LPARAM) as HTREEITEM
	}

	pub fn DeleteItem(&self,hItem: HTREEITEM,)->BOOL {
		self.cwin.SendMessage(TVM_DELETEITEM, 0, hItem as LPARAM) as BOOL
	}

	pub fn DeleteAllItems(&self,)->BOOL {
		self.cwin.SendMessage(TVM_DELETEITEM, 0, TVI_ROOT as LPARAM) as BOOL
	}
	...
	pub fn Expand(&self,hItem: HTREEITEM, nCode: Option<UINT> /*= TVE_EXPAND*/,)->BOOL {
		...
		self.cwin.SendMessage(TVM_EXPAND, n as WPARAM, hItem as LPARAM) as BOOL
	}

	pub fn GetNextItem(&self,hItem: HTREEITEM, nCode: UINT,)->HTREEITEM {
		 
		self.cwin.SendMessage(TVM_GETNEXTITEM, nCode as WPARAM, hItem as LPARAM) as HTREEITEM
	}

	pub fn GetChildItem(&self,hItem: HTREEITEM,)->HTREEITEM {
		 
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_CHILD, hItem as LPARAM) as HTREEITEM
	}

	pub fn GetNextSiblingItem(&self,hItem: HTREEITEM,)->HTREEITEM {
		 
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_NEXT, hItem as LPARAM) as HTREEITEM 
	}

	pub fn GetPrevSiblingItem(&self,hItem: HTREEITEM,)->HTREEITEM {
		 
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_PREVIOUS, hItem as LPARAM) as HTREEITEM
	}

	pub fn GetParentItem(&self,hItem: HTREEITEM,)->HTREEITEM {
		 
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_PARENT, hItem as LPARAM) as HTREEITEM 
	}

	pub fn GetFirstVisibleItem(&self,)->HTREEITEM {
		 
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_FIRSTVISIBLE, 0) as HTREEITEM
	}

	pub fn GetNextVisibleItem(&self,hItem: HTREEITEM,)->HTREEITEM {
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_NEXTVISIBLE, hItem as LPARAM) as HTREEITEM
	}

	pub fn GetPrevVisibleItem(&self,hItem: HTREEITEM,)->HTREEITEM {
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_PREVIOUSVISIBLE, hItem as LPARAM) as HTREEITEM
	}

	pub fn GetSelectedItem(&self,)->HTREEITEM {
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_CARET, 0) as HTREEITEM
	}

	pub fn GetDropHilightItem(&self,)->HTREEITEM {
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_DROPHILITE, 0) as HTREEITEM
	}

	pub fn GetRootItem(&self,)->HTREEITEM {
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_ROOT, 0) as HTREEITEM
	}

//#if !defined(_WIN32_WCE) && (_WIN32_IE >= 0x0400)
	pub fn GetLastVisibleItem(&self,)->HTREEITEM {
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_LASTVISIBLE, 0) as HTREEITEM
	}
//#endif // !defined(_WIN32_WCE) && (_WIN32_IE >= 0x0400)

//#if (_WIN32_IE >= 0x0600)
	pub fn GetNextSelectedItem(&self,)->HTREEITEM {
		self.cwin.SendMessage(TVM_GETNEXTITEM, TVGN_NEXTSELECTED, 0) as HTREEITEM
	}
//#endif // (_WIN32_IE >= 0x0600)

	pub fn Select(&self,hItem: HTREEITEM, nCode: UINT,)->BOOL {
		self.cwin.SendMessage(TVM_SELECTITEM, nCode as WPARAM, hItem as LPARAM) as BOOL
	}

	pub fn SelectItem(&self,hItem: HTREEITEM,)->BOOL {
		self.cwin.SendMessage(TVM_SELECTITEM, TVGN_CARET, hItem as LPARAM) as BOOL
	}

	pub fn SelectDropTarget(&self,hItem: HTREEITEM,)->BOOL {
		self.cwin.SendMessage(TVM_SELECTITEM, TVGN_DROPHILITE, hItem as LPARAM) as BOOL
	}

	pub fn SelectSetFirstVisible(&self,hItem: HTREEITEM,)->BOOL {
		self.cwin.SendMessage(TVM_SELECTITEM, TVGN_FIRSTVISIBLE, hItem as LPARAM) as BOOL
	}

	// pub fn EditLabel(&self,hItem: HTREEITEM,)->CEdit {
	// 	CEdit(self.cwin.SendMessage(TVM_EDITLABEL, 0, hItem as LPARAM) as HWND)
	// }

	pub fn EndEditLabelNow(&self,bCancel: BOOL,)->BOOL {
		self.cwin.SendMessage(TVM_ENDEDITLABELNOW, bCancel as WPARAM, 0) as BOOL
	}

	pub fn HitTest(&self,pHitTestInfo: *mut TVHITTESTINFO,)->HTREEITEM {
		self.cwin.SendMessage(TVM_HITTEST, 0, pHitTestInfo as LPARAM) as HTREEITEM
	}

	pub fn HitTest_point(&self,pt: POINT, pFlags: Option<&mut UINT>,)->HTREEITEM {
		let mut hti: TVHITTESTINFO = unsafe{mem::uninitialized()};
		hti.pt = pt;
		let hTreeItem = self.cwin.SendMessage(TVM_HITTEST, 0, &hti as *const _ as LPARAM) as HTREEITEM;
	...
	pub fn SortChildren(&self,hItem: HTREEITEM, bRecurse: Option<BOOL> /*= FALSE*/,)->BOOL {
		let b = if let Some(b1) = bRecurse {b1} else{ FALSE };
		self.cwin.SendMessage(TVM_SORTCHILDREN, b as WPARAM, hItem as LPARAM) as BOOL
	}

	pub fn EnsureVisible(&self,hItem: HTREEITEM,)->BOOL {
		self.cwin.SendMessage(TVM_ENSUREVISIBLE, 0, hItem as LPARAM) as BOOL
	}

	pub fn SortChildrenCB(&self,pSort: LPTVSORTCB, bRecurse: Option<BOOL> /*= FALSE*/,)->BOOL {
		let b = if let Some(b1) = bRecurse {b1} else{ FALSE };
		self.cwin.SendMessage(TVM_SORTCHILDRENCB, b as WPARAM, pSort as LPARAM) as BOOL
	}

	// pub fn RemoveImageList(&self,nImageList: c_int,)->CImageList {
	// 	CImageList(self.cwin.SendMessage(TVM_SETIMAGELIST, nImageList as WPARAM, NULL) as HIMAGELIST)
	// }

	// pub fn CreateDragImage(&self,hItem: HTREEITEM,)->CImageList {
	// 	CImageList(self.cwin.SendMessage(TVM_CREATEDRAGIMAGE, 0, hItem as LPARAM) as HIMAGELIST)
	// }

//#if (_WIN32_IE >= 0x0400) && !defined(_WIN32_WCE)
	pub fn SetInsertMark(&self,hTreeItem: HTREEITEM, bAfter: BOOL,)->BOOL {
		self.cwin.SendMessage(TVM_SETINSERTMARK, bAfter as WPARAM, hTreeItem as LPARAM) as BOOL
	}

	pub fn RemoveInsertMark(&self,)->BOOL {
		self.cwin.SendMessage(TVM_SETINSERTMARK, 0, 0) as BOOL
	}
//#endif // (_WIN32_IE >= 0x0400) && !defined(_WIN32_WCE)

//#if (_WIN32_WINNT >= 0x0501)
	pub fn MapAccIDToHTREEITEM(&self,uID: UINT,)->HTREEITEM {
		self.cwin.SendMessage(TVM_MAPACCIDTOHTREEITEM, uID as WPARAM, 0) as HTREEITEM
	}

	pub fn MapHTREEITEMToAccID(&self,hTreeItem: HTREEITEM,)->UINT {
		self.cwin.SendMessage(TVM_MAPHTREEITEMTOACCID, hTreeItem as WPARAM, 0) as UINT
	}
//#endif // (_WIN32_WINNT >= 0x0501)

//#if (_WIN32_WINNT >= 0x0600)
	pub fn ShowInfoTip(&self,hItem: HTREEITEM,) {
		self.cwin.SendMessage(TVM_SHOWINFOTIP, 0, hItem as LPARAM);
	}
//#endif // (_WIN32_WINNT >= 0x0600)
}
