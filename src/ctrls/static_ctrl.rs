...
//#ifndef _WIN32_WCE
	pub fn GetIcon (&self)->HICON {
		self.assert_window();
		self.SendMessage( STM_GETICON, 0, 0) as HICON
	}

	pub fn SetIcon(&self,hIcon: HICON)->HICON {
		self.assert_window();
		self.SendMessage( STM_SETICON, hIcon as WPARAM, 0) as HICON
	}

	pub fn GetEnhMetaFile (&self)->HENHMETAFILE {
		self.assert_window();
		self.SendMessage( STM_GETIMAGE, IMAGE_ENHMETAFILE as WPARAM, 0) as HENHMETAFILE
	}

	pub fn SetEnhMetaFile(&self,hMetaFile: HENHMETAFILE)->HENHMETAFILE {
		self.assert_window();
		self.SendMessage( STM_SETIMAGE, IMAGE_ENHMETAFILE as WPARAM, hMetaFile as LPARAM) as HENHMETAFILE
	}
//#else // CE specific
	// pub fn GetIcon (&self)->HICON {
	// 	self.assert_window();
	// 	self.SendMessage( STM_GETIMAGE, IMAGE_ICON, 0) as HICON
	// }

	// pub fn SetIcon(&self,hIcon: HICON)->HICON {
	// 	self.assert_window();
	// 	self.SendMessage( STM_SETIMAGE, IMAGE_ICON, hIcon as LPARAM) as HICON
	// }
//#endif // _WIN32_WCE

	// pub fn GetBitmap ()->CBitmapHandle {
	// 	self.assert_window();
	// 	return CBitmapHandle((HBITMAP)self.SendMessage( STM_GETIMAGE, IMAGE_BITMAP, 0));
	// }

	// pub fn SetBitmap(&self,hBitmap: HBITMAP)->CBitmapHandle {
	// 	self.assert_window();
	// 	return CBitmapHandle((HBITMAP)self.SendMessage( STM_SETIMAGE, IMAGE_BITMAP, hBitmap as LPARAM));
	// }

	pub fn GetCursor (&self)->HCURSOR {
		self.assert_window();
		self.SendMessage( STM_GETIMAGE, IMAGE_CURSOR as WPARAM, 0) as HCURSOR
	}

	pub fn SetCursor(&self,hCursor: HCURSOR)->HCURSOR {
		self.assert_window();
		self.SendMessage( STM_SETIMAGE, IMAGE_CURSOR as WPARAM, hCursor as LPARAM) as HCURSOR
	}
}
