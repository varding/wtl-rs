...
#[macro_export]
macro_rules! impl_proc_msg {
	() => {
		pub fn ProcessWindowMessage(p_this:*mut c_void,hWnd:HWND,uMsg:UINT,wParam:WPARAM,lParam:LPARAM,lResult:&mut LRESULT,dwMsgMapID:DWORD ) -> BOOL {
			let p = p_this as *mut Self;
			let mut_self = unsafe{&mut *p};
			let pself = unsafe{&*p};

			mut_self.handler.disptach_msg(pself,hWnd,uMsg,wParam,lParam,lResult,dwMsgMapID)
		}
	};
	($f:ident) => {
		pub fn ProcessWindowMessage(p_this:*mut c_void,hWnd:HWND,uMsg:UINT,wParam:WPARAM,lParam:LPARAM,lResult:&mut LRESULT,dwMsgMapID:DWORD ) -> BOOL {
...
impl CDialogImpl {
	...
	unsafe extern "system" fn DialogProc(hWnd:HWND,uMsg:UINT,wParam:WPARAM,lParam:LPARAM) -> INT_PTR {
		...
		let mut bRet = (mut_self.proc_msg)(mut_self.puser,hWnd,uMsg,wParam,lParam,&mut lRes,0);//unsafe{};
		if bRet == TRUE {
			match uMsg{
...
//CDialogImpl
impl CDialogImpl {
	pub fn DoModal2(&mut self,puser:*mut c_void){
		 let hWndParent = unsafe{user32::GetActiveWindow()};
		 self.DoModal(puser,hWndParent, NULL_LPARAM);
	}
	
	pub fn DoModal(&mut self,puser :*mut c_void,hWndParent:HWND,dwInitParam:LPARAM) -> INT_PTR {
	...
	// modeless dialogs
	pub fn Create3(&mut self,puser: *mut c_void){
		self.Create(puser,NULL_HWND, NULL_LPARAM);
	}

	pub fn Create2(&mut self,puser: *mut c_void,hWndParent: HWND){
		self.Create(puser,hWndParent, NULL_LPARAM);
	}

	pub fn Create(&mut self,puser:*mut c_void,hWndParent:HWND,dwInitParam:LPARAM) -> HWND {
		//ATLASSUME(m_hWnd == NULL);
		self.puser = puser 	;
		thunk::set_this(self as *mut Self as *mut c_void);
